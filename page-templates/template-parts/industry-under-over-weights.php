...
if ( empty( $weighting_data_id ) ) {
    error_log( 'Weighting data file attachment ID is empty or unavailable.' );
    return;
}
...
if ( ! file_exists( $file_path ) ) {
    error_log( 'Weighting data file does not exist at the specified path: ' . $file_path );
    return;
}
...
if ( false === $fp ) {
    error_log( 'Failed to open weighting data file.' );
    return;
}
